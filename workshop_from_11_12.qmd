...
df %>% 
    ...
    summarise(mean_bpm = mean(bpm, na.rm = T),
              std_dev_bpm = sd(bpm, na.rm = T),
              median_bpm = median(bpm, na.rm = T),
              sample_size = n()
              )
```
...
Using `na.rm = T` is sort of annoying, can we do it differently?
...
# This breaks on purpose
df %>%
    select(streams, mode, released_month, bpm, key, danceability, energy, liveness) %>%
    filter(!is.na(danceability),
           !is.na(bpm)
           ) %>%
    mutate(
        # rel_mnth_chr = as.character(released_month),
        streams_mill = streams/1000000,
        is_summer = ifelse(released_month %in% c(6, 7, 8), "Yes","No")
    ) %>%
    select(streams_mill, mode, is_summer, bpm, key, energy, danceability) -> df_new
```
...
Let's give chatGPT a go with this one: How can you make a boxplot using ggplot2? Mention your grouping and response variables.
...
ggplot(data = df_new,
    ...
    geom_jitter(width = .15, color = 'blue', alpha = .4,
                size = .5) +
    theme_bw()

ggsave('images/myboxplot.png')
```
